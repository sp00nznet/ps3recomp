                          ...
                          const char* threadname)
{
    ...
    printf("[sysPrxForUser] sys_ppu_thread_create(entry=%p, prio=%d, "
           "stack=0x%X, name='%s')\n",
           (void*)(uintptr_t)entry, priority, stacksize,
    ...
    int idx = -1;
    for (int i = 0; i < MAX_PRX_THREADS; i++) {
        if (!s_threads[i].in_use) { idx = i; break; }
    }
    if (idx < 0)
        return CELL_EAGAIN;

    PrxThreadSlot* t = &s_threads[idx];
    memset(t, 0, sizeof(*t));
    t->in_use = 1;
    t->thread_id = s_next_thread_id++;
    t->entry = entry;
    t->arg = arg;
...
#ifdef _WIN32
    t->handle = CreateThread(NULL, stacksize, prx_thread_entry, t, 0, NULL);
    if (!t->handle) {
        t->in_use = 0;
        return CELL_EAGAIN;
    }
    ...
    int rc = pthread_create(&t->pt, &attr, prx_thread_entry, t);
...
s32 sys_ppu_thread_join(sys_ppu_thread_t thread_id, u64* exitstatus)
{
    ...
    for (int i = 0; i < MAX_PRX_THREADS; i++) {
        if (s_threads[i].in_use && s_threads[i].thread_id == thread_id) {
            PrxThreadSlot* t = &s_threads[i];
#ifdef _WIN32
...
{
    printf("[sysPrxForUser] sys_ppu_thread_detach(id=%llu)\n",
           (unsigned long long)thread_id);

    for (int i = 0; i < MAX_PRX_THREADS; i++) {
        if (s_threads[i].in_use && s_threads[i].thread_id == thread_id) {
            s_threads[i].detached = 1;
#ifdef _WIN32
            CloseHandle(s_threads[i].handle);
            s_threads[i].handle = NULL;
#else
            pthread_detach(s_threads[i].pt);
#endif
            return CELL_OK;
        }
    }
...
s32 sys_ppu_thread_get_id(sys_ppu_thread_t* thread_id)
{
    ...
    for (int i = 0; i < MAX_PRX_THREADS; i++) {
        if (s_threads[i].in_use) {
            *thread_id = s_threads[i].thread_id;
            return CELL_OK;
        }
    }
    *thread_id = (u64)tid;
