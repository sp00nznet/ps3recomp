 ...
 *
 * PNG decoding using stb_image.h (place stb_image.h in include/).
 * If stb_image.h is not available, returns CELL_ENOSYS for decode operations.
 *
 * To enable: #define STB_IMAGE_IMPLEMENTATION in exactly one TU that includes
 * stb_image.h, or define PS3RECOMP_HAS_STB_IMAGE before including this file
 * in a build that already has stb_image compiled.
 */
...
typedef struct {
    ...
    int               header_read;
    int               param_set;
    /* Source data loaded into memory */
    u8*               src_data;
    u32               src_size;
} PngDecSubState;
...
static int pngdec_load_source(PngDecSubState* sub)
{
    ...
    if (sub->src.srcSelect == CELL_PNGDEC_BUFFER) {
        /* Source is a memory buffer. In a full emulator, we'd translate
         * the guest address through vm::g_base. For now, treat streamPtr
         * as a host pointer. */
    ...
    if (sub->src.srcSelect == CELL_PNGDEC_FILE) {
         ...
         * In a full emulator, this would go through cellFs path translation. */
        FILE* f = fopen(sub->src.fileName, "rb");
        if (!f) {
            printf("[cellPngDec] Cannot open file: %s\n", sub->src.fileName);
            return -1;
        }
...
{
    /* Only free if we allocated (file mode) */
    if (sub->src.srcSelect == CELL_PNGDEC_FILE && sub->src_data) {
        free(sub->src_data);
    }
    sub->src_data = NULL;
    sub->src_size = 0;
...
s32 cellPngDecDestroy(CellPngDecMainHandle mainHandle)
{
    ...
    /* Close any sub-handles belonging to this main handle */
...
s32 cellPngDecReadHeader(CellPngDecMainHandle mainHandle,
                         ...
                         CellPngDecInfo* info)
{
    printf("[cellPngDec] ReadHeader(main=%u, sub=%u)\n", mainHandle, subHandle);
...
#if PNGDEC_HAS_STB
    {
        ...
        if (!stbi_info_from_memory(sub->src_data, (int)sub->src_size, &w, &h, &comp)) {
            printf("[cellPngDec] stbi_info failed\n");
...
#else
    /* Minimal PNG header parsing without stb_image */
...
{
    (void)mainHandle;
    printf("[cellPngDec] SetParameter(sub=%u)\n", subHandle);
                         ...
                         CellPngDecDataInfo* dataInfo)
{
...
#if PNGDEC_HAS_STB
    {
        ...
        /* Handle ARGB output: stb gives us RGBA, swap to ARGB */
...
#else
    printf("[cellPngDec] DecodeData: stb_image not available, cannot decode\n");
    dataInfo->status = CELL_PNGDEC_DEC_STATUS_STOP;
